...
export default class EventDragging extends Interaction {
  ...
  */
  ...
  computeEventDateMutation(startFootprint, endFootprint) {
    ...
    if(dateDelta._milliseconds === 0){
      date0 = startFootprint.unzonedRange.getEnd();
      date1 = endFootprint.unzonedRange.getEnd();
      dateDelta = this.component.diffDates(date1, date0);
      dateDelta.first = 'first cell';
  }
    ...
    if (dateMutation.dateDelta !== null && (endFootprint.unzonedRange.extra === 'pm' || startFootprint.unzonedRange.extra === 'pm')) {
      if (startFootprint.unzonedRange.extra === 'pm') {
          if(endFootprint.unzonedRange.extra === 'am'){
              dateMutation.dateDelta.first = 'first cell';
          }
          dateMutation.dateDelta.extra = '-pm';
      }
      else {
          dateMutation.dateDelta.extra = 'pm';
      }
  ...
  else if (dateMutation.dateDelta !== null && (endFootprint.unzonedRange.extra === 'am' || startFootprint.unzonedRange.extra === 'am')) {
      if (startFootprint.unzonedRange.extra === 'am') {
          dateMutation.dateDelta.extra = '-am';
      }
      else {
          dateMutation.dateDelta.extra = 'am';
      }
