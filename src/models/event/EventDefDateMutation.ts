  ...
  buildNewDateProfile(eventDateProfile, calendar) {
    ...
    if (this.dateDelta) {
      if (this.dateDelta.extra && (this.dateDelta.extra === 'pm' || this.dateDelta.extra === 'am' || this.dateDelta.extra === '-pm' || this.dateDelta.extra === '-am')) {
          ...
          if (this.dateDelta.extra === 'pm') {
              start = calendar.currentDate.clone();
              start.add(43200000);
              end = dummy.add(43200000);
          }
          else if (this.dateDelta.extra === '-pm') {
              if (this.dateDelta.first === 'first cell'){
                  start =  calendar.currentDate.clone();
                  end = calendar.currentDate.clone().add(43200000);
              }else{
              start.add(this.dateDelta);
              end = eventDateProfile.start.clone().add(this.dateDelta);
              // end.add(-43200000);
              end.add(1800000);
          }
      }
          else if (this.dateDelta.extra === 'am') {
              start = calendar.currentDate.clone();
              end = calendar.currentDate.clone().add(43200000);
          }
          else if (this.dateDelta.extra === '-am') {
              if (this.dateDelta.first === 'first cell'){
                  start = calendar.currentDate.clone();
                  end = calendar.currentDate.clone().add(1800000);
              }else{
              start.add(this.dateDelta);
              end = eventDateProfile.start.clone().add(this.dateDelta);
              end.add(1800000);
